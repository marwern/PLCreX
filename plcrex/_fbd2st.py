...
def translation(src: Path,
    ...
    def write_header(pou_type_plc):
        ...
        for i1 in root.iter('{http://www.plcopen.org/xml/tc6_0201}pou'):
            ...
            st_file.write(pou_type_plc + ' ' + pou_name + '\n')
        ...
        for i1 in root.iter('{http://www.plcopen.org/xml/tc6_0201}block'):
            ...
            if i1.get('instanceName') is not None:
                    ...
                    for i3 in i2.iter('{http://www.plcopen.org/xml/tc6_0201}variable'):
                        for i4 in i3.iter('{http://www.plcopen.org/xml/tc6_0201}connection'):
                            if i4.get('formalParameter'):
                                inputVariables_list.append({"formalParameter": i3.get('formalParameter'),
                ...
                for i2 in i1.iter('{http://www.plcopen.org/xml/tc6_0201}outputVariables'):
                    # for each output
                    for i3 in i2.iter('{http://www.plcopen.org/xml/tc6_0201}variable'):
                        outputVariables_list.append({"formalParameter": i3.get('formalParameter')})

                # append dictionary
                tmp = {"localId": i1.get('localId'),
                       "instanceName": i1.get('instanceName'),
                       "inputVariables": inputVariables_list,
                       "outputVariables": outputVariables_list,
                ...
                for i2 in i1.iter('{http://www.plcopen.org/xml/tc6_0201}outputVariables'):
                    ...
                    for i3 in i2.iter('{http://www.plcopen.org/xml/tc6_0201}variable'):
                        print('outputVariables = {formalParameter: ' + i3.get(
                            'formalParameter') + ', newVariable: ' + i1.get('typeName') + i1.get('localId') + "_" + i3.get(
                            'formalParameter') + '}')
                        outputVariables_list.append(
                            {"formalParameter": i3.get('formalParameter'),
    ...
    def create_interface_with_Id():
        ...
        for i1 in root.iter('{http://www.plcopen.org/xml/tc6_0201}outVariable'):
            ...
            for i2 in i1.iter('{http://www.plcopen.org/xml/tc6_0201}connection'):
                tmp_refLocalId = i2.get("refLocalId")
                tmp_formalParameter = i2.get("formalParameter")

            print(tmp_name, 'localId', i1.get('localId'), "outVariable", tmp_refLocalId, tmp_formalParameter)  # Debug
            interfaces_with_ids.append({"localId": i1.get('localId'),
                                        "expression": tmp_name,
                                        "dir": "outVariable",
                                        "refLocalId": tmp_refLocalId,
                                        "formalParameter": tmp_formalParameter})
    ...
    def create_fbd_connections():
        ...
        for dict_pou in block_comp:
            ...
            if dict_pou.get("typeName") is not None:
                ...
                for input in dict_pou["inputVariables"]:
                    print(input)

                    # check if POU/FB input port is connected to interface / local variables
                    for inst in interfaces_with_ids:
                        if input["refLocalId"] == inst["localId"]:
                            print("..connected to interface / local variables")
                            tmp = inst["expression"]
                            print(tmp)
                            arguments.append(tmp)

                    # check if POU/FB input port is connected to other POU/FB output port
                        ...
                        if input["refLocalId"] == inst["localId"]:
                            if dict_pou.get("typeName"):  # instanceName
                                ...
                                if inst.get("instanceName") is None:
                                    ...
                                    tmp = inst["typeName"] + inst["localId"] + '_' + input["formalParameter_port"]
                                # instantiated function block
                                else:
                                    print("..connected to other POU/FB output port")
                                    tmp = inst["instanceName"] + '.' + input["formalParameter_port"]
                                print(tmp)
                                arguments.append(tmp)
            ...
            # check if dict_pou is a instantiated function block
                ...
                for input in dict_pou["inputVariables"]:
                    ...
                    for inst in interfaces_with_ids:
                        if input["refLocalId"] == inst["localId"]:
                            tmp = input["formalParameter"] + ' := ' + inst["expression"]
                            print(tmp)
                            arguments.append(tmp)
                    ...
                    for inst in block_comp:
                        ...
                        if input["refLocalId"] == inst["localId"]:
                            ...
                            if inst.get("instanceName"):
                                tmp = input["formalParameter"] + ' := '+inst["instanceName"] + '.' + input["formalParameter_port"]
                                print(tmp)
                                arguments.append(tmp)

                            # connected to non instantiated function block
                            elif inst.get("typeName"):
                                tmp = input["formalParameter"] + ' := '+ inst["typeName"] + inst.get("localId")+ '_' + input["formalParameter_port"]
                                print(tmp)
                                arguments.append(tmp)
                                variables_to_declare.append({
                                    "typeName": inst.get("typeName"),
                                    "localId": inst.get("localId"),
                                    "formalParameter": input["formalParameter_port"],
    ...
    def create_output_connections():
        ...
        for outVar in interfaces_with_ids:
            if outVar.get("dir") == "outVariable":
                ...
                # find connected block output (if not connected to input variable)
                if not tmp_finding:
                    for block in block_comp:
                        if outVar.get("refLocalId") == block.get("localId"):
                            if block.get("instanceName") is not None:
                                tmp_stat = outVar.get("expression") + ' := ' + block.get("instanceName") + '.' + outVar.get(
                                    "formalParameter") + ';'
    ...
    with open(fr'.\exports\st\{Path(src).name}.st', 'w') as st_file:
        pou_type_plc = "undefined" # pou name
